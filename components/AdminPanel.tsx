...
import { UserPlus, Trash2, Play, RefreshCw, Copy, Check, ExternalLink, ArrowLeft, Link as LinkIcon } from 'lucide-react';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const copyLinkToClipboard = (p: Participant) => {
    ...
    const text = `Oi ${p.name}! Aqui está o link para ver quem você tirou no Amigo Oculto (${event.name}):\n${url}`;
    navigator.clipboard.writeText(text);
              ...
              <tr>
                ...
                {!event.isDrawn ? (
                  ...
                  <>
                    ...
                    <th className="px-6 py-4">Tirou</th>
                    <th className="px-6 py-4 text-right">Ações de Compartilhamento</th>
                  </>
                )}
              </tr>
            </thead>
                ...
                event.participants.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    {!event.isDrawn ? (
                      ...
                      <>
                        <td className="px-6 py-4 font-mono text-indigo-600 font-bold uppercase">{p.password}</td>
                        <td className="px-6 py-4 text-gray-600">{getDrawnName(p.drawnId)}</td>
